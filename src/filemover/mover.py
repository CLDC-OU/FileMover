...
class Mover:
    ...
    def move_files(self):
        ...
        messages = []
        if moved_files > 0:
            messages.append(f"Moved {moved_files} file{'' if moved_files == 1 else 's'}")
        if copied_files > 0:
            messages.append(f"Copied {copied_files} file{'' if copied_files == 1 else 's'}")
        if deleted_files > 0:
            messages.append(f"Deleted {deleted_files} file{'' if deleted_files == 1 else 's'}")
